...
namespace LightEditor2.Maui.Services // Namespace des MAUI-Projekts
{
    public class MauiFileSaverService : IFileSaverService
    {
        ...
        public async Task<FileSaverResult> SaveAsync(string initialPath, Stream stream, CancellationToken cancellationToken = default)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return new FileSaverResult { IsSuccessful = false, Exception = ex };
            }
        }

        public async Task<FileSaverResult> SaveAsync(Stream stream, CancellationToken cancellationToken = default)
        {
